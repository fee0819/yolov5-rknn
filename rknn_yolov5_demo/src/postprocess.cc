                 ...
                 detect_result_group_t *group)
{
    ...
    std::ofstream out1("1_tensorrt.txt", std::ios::out);
    if (out1.is_open())
    {
        for (int ID = 0; ID < 21 * 48 * 80; ID++)
        {
            int8_t *in_ptr = input0 + ID;
            out1 << deqnt_affine_to_f32(*in_ptr, qnt_zps[0], qnt_scales[0]) << std::endl;
        }
        out1.close();
    }
